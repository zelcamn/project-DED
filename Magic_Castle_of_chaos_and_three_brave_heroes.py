...
class Music:
    pygame.mixer.music.load('data/music/gamemus1.mp3')  # загружем музыку, по другому я не мгогу её остановить
    gamemusic = pygame.mixer.music
    pygame.mixer.music.set_volume(0.5)
    shoot = pygame.mixer.Sound('data/music/shot.wav')  # звуки можно не загружать
    povrejen = pygame.mixer.Sound('data/music/povrejen.mp3')
    gameover = pygame.mixer.Sound('data/music/gameover.mp3')
    zlovded = pygame.mixer.Sound('data/music/zlovded.mp3')
    healthup = pygame.mixer.Sound('data/music/healthup.mp3')
    meinmusic = pygame.mixer.Sound(f"data/music/m{random.randint(1, 3)}.mp3")
...
class Objectile(pygame.sprite.Sprite):
    def __init__(self, pos, a_x, time_of_live, damage):
...
class Objectile_Sword(Objectile):
    def __init__(self, pos, a_x, time_of_live, heigth, target_pos, real, damage):
        ...
        super().__init__(pos, a_x, time_of_live, damage)
...
class Level:
    ...
    def create_platform(self, widht_of_platform, heigth, pos, image):
        Platform(widht_of_platform - 2, 1, (pos[0] + 1, pos[1]), self.vertical_platforms_up)
        Platform(widht_of_platform - 2, 1,
        ...
        Platform(1, heigth - 2,
                 (pos[0] + widht_of_platform - 1, pos[1] + 1),
...
class Camera(object):
    def __init__(self, camera_func, width_of_camera_rect, height_of_camera_rect):
        self.camera_func = camera_func
        self.state = pygame.Rect(0, 0, width_of_camera_rect, height_of_camera_rect)
...
def menu():
    ...
    main_menu = Menu(surf, [[load_image("play.png", colorkey="white"), 150, 50, start_play],
...
def pause():
    ...
    surf.image = load_image("fone.png")
    surf.rect = surf.image.get_rect()
    pause_menu = Menu(surf, [[load_image("pause.png", colorkey="white"), 150, 50, unpause],
    ...
    while running_pause:
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                running_pause = False
            if event.type == pygame.MOUSEBUTTONDOWN:
                pause_menu.update(event.pos)
        pause_menu.render(sc)
        pygame.display.flip()
...
def dead_screen():
    ...
    surf.rect = surf.image.get_rect()
    menu_dead_screen = Menu(surf, [[load_image("next.png", colorkey="white"), 200, 450, stop_dead_screen]])
    ...
    while dead_screen_run:
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                dead_screen_run = False
            if event.type == pygame.MOUSEBUTTONDOWN:
                menu_dead_screen.update(event.pos)
...
def aboute_razrabotchikav():
    ...
    surf.image = load_image("fone.png")
    surf.rect = surf.image.get_rect()
    menu_aboute_razrabotchikav = Menu(surf, [[load_image("next.png", colorkey="white"), 210, 450, stop_aboute_razrabotchikav]])
    ...
    while aboute_razrabotchikav_run:
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                aboute_razrabotchikav_run = False
            if event.type == pygame.MOUSEBUTTONDOWN:
                menu_aboute_razrabotchikav.update(event.pos)
        if counter1 <= len(text[0]) * 10:
            counter1 += 1
        else:
            if counter2 <= len(text[1]) * 10:
                ...
                if counter3 <= len(text[2]) * 10:
                    ...
                    if counter4 <= len(text[3]) * 10:
                        counter4 += 1
        menu_aboute_razrabotchikav.render(sc)
        f1 = pygame.font.Font(None, 36)
        text1 = f1.render(text[0][:counter1 // 10], True,
                          (180, 180, 0))
        text2 = f1.render(text[1][:counter2 // 10], True,
